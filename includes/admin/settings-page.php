...
function ata_textarea_callback( $args ) {
	...
	$html  = sprintf( '<textarea class="large-text" cols="50" rows="20" id="ata_settings[%1$s]" name="ata_settings[%1$s]">%2$s</textarea>', sanitize_key( $args['id'] ), esc_textarea( stripslashes( $value ) ) );
	$html .= '<p class="description">' . wp_kses_post( $args['desc'] ) . '</p>';
